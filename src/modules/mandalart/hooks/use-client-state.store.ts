/* eslint-disable no-unused-vars */
import { create } from 'zustand';
import {
  CellTodo,
  MandalartCore,
  MandalartSubtopic,
  MandalartTopic,
  MandalartAllJson,
} from '../types/realtime-type';

type MapStore = {
  core: Map<string, MandalartCore>;
  topics: Map<string, MandalartTopic>;
  subTopics: Map<string, MandalartSubtopic>;
  todos: Map<string, CellTodo>;

  initialize: (data: MandalartAllJson) => void;

  setCoreItem: (key: string, item: MandalartCore) => void;
  getCoreItem: (key: string) => MandalartCore | undefined;

  setTopicItem: (key: string, item: MandalartTopic) => void;
  getTopicItem: (key: string) => MandalartTopic | undefined;

  setSubTopicItem: (key: string, item: MandalartSubtopic) => void;
  getSubTopicItem: (key: string) => MandalartSubtopic | undefined;

  setTodoItem: (key: string, item: CellTodo) => void;
  getTodoItem: (key: string) => CellTodo | undefined;
  removeTodoItem: (key: string) => void;

  clear: () => void;
};

export const useClientStateStore = create<MapStore>((set, get) => ({
  core: new Map(),
  topics: new Map(),
  subTopics: new Map(),
  todos: new Map(),

  //initialize
  initialize: (fetchData) => {
    const coreMap = new Map([[fetchData.core.id, fetchData.core]]);
    set({ core: coreMap });
    const topicMap = new Map(fetchData.topics.map((item) => [item.id, item]));
    set({ topics: topicMap });
    const subTopicMap = new Map(
      fetchData.subtopics.map((item) => [item.id, item])
    );
    set({ subTopics: subTopicMap });
    const todoMap = new Map(fetchData.todos.map((item) => [item.id, item]));
    set({ todos: todoMap });
  },

  // core
  setCoreItem: (key, item) => {
    const newMap = new Map(get().core);
    newMap.set(key, item);
    set({ core: newMap });
  },
  getCoreItem: (key) => get().core.get(key),

  // topics
  setTopicItem: (key, item) => {
    const newMap = new Map(get().topics);
    newMap.set(key, item);
    set({ topics: newMap });
  },
  getTopicItem: (key) => get().topics.get(key),

  // subTopics
  setSubTopicItem: (key, item) => {
    const newMap = new Map(get().subTopics);
    newMap.set(key, item);
    set({ subTopics: newMap });
  },
  getSubTopicItem: (key) => get().subTopics.get(key),

  // todos
  setTodoItem: (key, item) => {
    const newMap = new Map(get().todos);
    newMap.set(key, item);
    set({ todos: newMap });
  },
  getTodoItem: (key) => get().todos.get(key),
  removeTodoItem: (key) => {
    const newMap = new Map(get().todos);
    newMap.delete(key);
    set({ todos: newMap });
  },

  // 공통 초기화
  clear: () => {
    set({
      core: new Map(),
ew Map(),
);
