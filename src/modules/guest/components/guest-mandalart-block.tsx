  ...
  return (
    <div className='grid aspect-square grid-cols-3 grid-rows-3 gap-2'>
      ...
      {Array.from({ length: 8 }).map((_, index) => (
        <div
          key={index}
          className={`${!activeSub ? 'bg-white-light' : 'bg-lightgray'} ${delayWithIndex(index)} relative flex aspect-square max-w-full items-center justify-center overflow-hidden rounded-lg border border-caption p-2 transition-all delay-[400ms] duration-500`}
        >
